...
█▀▀ ░█░ ██▄ █▀▄ █▄█ █▄▀ █▀█ █▄▄ ░█░ ░█░ █▄▄
EOF
    echo -e "         ${WHITE}⚡ PREMIUM PTERODACTYL AUTO-INSTALLER ⚡${NC}"
    echo -e "${HEADER_LINE}"
}
...
select_version() {
    ...
    if [[ ${#tags[@]} -eq 0 ]]; then
        echo -e "  ${GOLD}No versions found. Using latest.${NC}"
        eval "$var_name=\"$default\""
        return
    fi
...
echo -e "  ${GOLD}║${NC} ${GRAY}Domain  :${NC}  ${WHITE}$DOMAIN${NC}"
echo -e "  ${GOLD}║${NC} ${GRAY}Email   :${NC}  ${WHITE}$EMAIL${NC}"
echo -e "  ${GOLD}║${NC} ${GRAY}User    :${NC}  ${WHITE}$USERNAME${NC}"
echo -e "  ${GOLD}║${NC} ${GRAY}Version :${NC}  ${WHITE}$version_PANEL${NC}"
echo -e "  ${GOLD}╚════════════════════════════════════════════════════════╝${NC}"
...
# --- Dependencies ---
step "Updating system & installing dependencies..."
...
if [[ "$OS" == "ubuntu" ]]; then
    info "Detected Ubuntu. Adding PPA for PHP..."
    apt install -y software-properties-common
    LC_ALL=C.UTF-8 add-apt-repository -y ppa:ondrej/php
elif [[ "$OS" == "debian" ]]; then
    info "Detected Debian. Adding SURY PHP repo..."
...
step "Generating application key & running migrations..."
...
# --- Nginx Setup ---
step "Configuring Nginx & SSL Certificate..."
...
ok "Queue worker running successfully"

step "Configuring environment & caching..."
cd /var/www/pterodactyl
...
sed -i 's|MAIL_FROM_NAME=.*|MAIL_FROM_NAME="Pterodactyl"|g' .env
...
echo -e "  ${GRAY}────────────────────────────────────────${NC}"
echo -e "\n  ${PURPLE}Enjoy your new Apotheosis Pterodactyl Panel!${NC}"
echo -e "${HEADER_LINE}"
